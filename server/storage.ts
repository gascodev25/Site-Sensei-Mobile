...
export class DatabaseStorage implements IStorage {
  ...
  async getDailyStockForecast(customStartDate?: string): Promise<{
    ...
    // Calculate 28-day date range (4 weeks)
    ...
    if (allServices.length === 0) {
      ...
      for (let dayOffset = 0; dayOffset < 28; dayOffset++) {
        ...
        days.push({
          ...
          displayDate: currentDay.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }),
          consumables: [],
        });
      }
      return days;
    ...
    for (let dayOffset = 0; dayOffset < 28; dayOffset++) {
      ...
      const dayServices = allServices.filter(s => {
        ...
        // For recurring services, check if they have occurrences on this day
        ...
        if (isRecurring) {
          ...
          // Check if this specific day is an occurrence
          const completedDatesSet = new Set((s.completedDates as string[]) || []);
          const dateStr = currentDay.toISOString().split('T')[0];
          
          // Calculate if this day is an occurrence
          let currentOccurrence = new Date(serviceDate);
          const maxIterations = 1000;
          let iterations = 0;
          
          while (currentOccurrence <= dayEnd && iterations < maxIterations) {
            iterations++;
            
            if (currentOccurrence.toISOString().split('T')[0] === dateStr) {
              // This day is an occurrence - check if not completed
              return !completedDatesSet.has(dateStr);
            }
            
            currentOccurrence = new Date(currentOccurrence);
            currentOccurrence.setDate(currentOccurrence.getDate() + intervalDays);
          }
      ...
      // Calculate consumables needed for all services on this day
