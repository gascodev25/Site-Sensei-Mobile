        ...
        <TabsContent value="forecast" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-6">
              {(forecastView === 'weekly' && isLoadingForecast) || (forecastView === 'daily' && isLoadingDailyForecast) ? (
                ...
                dailyForecast && dailyForecast.length > 0 ? (
                  dailyForecast.map((day: any, index: number) => (
                    <div key={day.date} className="space-y-2" data-testid={`forecast-day-${index}`}>
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-semibold">
                          {format(new Date(day.date), 'EEE, MMM d')} <span className="text-sm text-muted-foreground">({day.dayOfWeek})</span>
                        </h3>
                        <Badge variant="outline">
